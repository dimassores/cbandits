...
class UCB_B1(BaseBanditAlgorithm):
    ...
    def __init__(self, num_arms, arm_configs, algorithm_params):
        ...
        self.b_min_cost = algorithm_params.get('b_min_cost', 0.1) # Small positive constant for denominator stability [cite: 85]

        # Initialize arm-specific statistics
        self.arm_pulls = np.zeros(num_arms)             # T_k(n) [cite: 85]
        self.total_costs = np.zeros(num_arms)           # Sum of X_i,k
        self.total_rewards = np.zeros(num_arms)         # Sum of R_i,k

        # Store known second-order moments [cite: 89]
        ...
        # Calculate omega_k and V(X, R) (Eq. 8, 9) [cite: 88, 89]
        ...
        for k in range(num_arms):
            if self.var_X[k] > 1e-9: # Avoid division by zero if variance is zero
                self.omega_k[k] = self.cov_XR[k] / self.var_X[k] [cite: 88]
            else:
                self.omega_k[k] = 0.0 # If Var(X)=0, omega is undefined, can be treated as 0 for practical purposes
            
            # V(X_1,k, R_1,k) = Var(R_1,k) - omega_k^2 * Var(X_1,k) [cite: 88]
            # If Var(X_1,k)=0, then V(X_1,k, R_1,k) = Var(R_1,k) [cite: 89]
            self.V_XR[k] = self.var_R[k] - (self.omega_k[k]**2 * self.var_X[k]) if self.var_X[k] > 1e-9 else self.var_R[k] [cite: 88, 89]
            # Ensure V_XR is non-negative due to potential floating point inaccuracies
            self.V_XR[k] = max(0, self.V_XR[k])

        # M_X and M_R for bounded case (if applicable, from Theorem 1.1) [cite: 93]
        # For jointly Gaussian, M_X=M_R=0 is stated[cite: 95], but for practical UCB bounds,
        ...
        # As per the paper, for jointly Gaussian, M_X=M_R=0 [cite: 95]
        self.M_X = algorithm_params.get('M_X', 0.0) # Used in specific UCB-B1 bound, set to 0 for Gaussian [cite: 95]
        self.M_R = algorithm_params.get('M_R', 0.0) # Used in specific UCB-B1 bound, set to 0 for Gaussian [cite: 95]
    ...
    def select_arm(self, current_total_cost, current_epoch):
        ...
        for k in range(self.num_arms):
            ...
            # Empirical estimates for mu_X and mu_R [cite: 85]
            ...
            # Rate estimator [cite: 85]
            # max(0, emp_mean_R) to ensure non-negative reward part
            # max(self.b_min_cost, emp_mean_X) to ensure positive denominator and stability [cite: 85]
            r_hat_k = max(0, emp_mean_R) / max(self.b_min_cost, emp_mean_X) [cite: 85]

            # Calculate epsilon_k,n^g and eta_k,n^g for the confidence bound [cite: 89]
            # M_R and M_X are considered 0 for the Jointly Gaussian case in Theorem 1.2 [cite: 95]
            epsilon_k_n_g = (2 * self.alpha * self.M_R * log_n / (3 * T_k)) + \
                            np.sqrt(self.L * self.alpha * self.V_XR[k] * log_n / T_k) [cite: 89]
            
            eta_k_n_g = (2 * self.alpha * self.M_X * log_n / (3 * T_k)) + \
                        np.sqrt(self.L * self.alpha * self.var_X[k] * log_n / T_k) [cite: 89]

            # Stability condition check: eta < theta_1 * (lambda - 1) / lambda [cite: 82]
            # Here, theta_1 is emp_mean_X and lambda = 1.28 [cite: 90]
            ...
            if emp_mean_X < self.b_min_cost: # If emp_mean_X is too close to zero
                ...
                # The condition is η < θ₁ for λ > 1[cite: 82, 85].
                # The term (lambda-1)/lambda is close to 1 for lambda=1.28.
                # A simpler interpretation of the stability condition (Remark 1) is η < θ₁ [cite: 85]
                # Let's use the explicit condition from Proposition 2 and the context around it.
                # Proposition 2 condition: eta_k_n_g < emp_mean_X * (lambda - 1) / lambda with lambda = 1.28 [cite: 82, 90]
                lambda_val = 1.28
                if eta_k_n_g >= emp_mean_X * (lambda_val - 1) / lambda_val:
                    stability_condition_met = False
            
            if not stability_condition_met:
                c_k_n_b1 = np.inf # Set confidence bound to infinity if stability condition not met [cite: 90]
            else:
                # Calculate the confidence bound term c_k,n^b1 [cite: 90]
                # Note: (r_hat_k - omega_k) for the epsilon term in the numerator [cite: 90]
                # Ensure the denominator (empirical_mean_X) is positive and stable
                c_k_n_b1_numerator = epsilon_k_n_g + (r_hat_k - self.omega_k[k]) * eta_k_n_g [cite: 90]
                c_k_n_b1_denominator = max(self.b_min_cost, emp_mean_X) # (E_n[X_k])^+ [cite: 90]
                c_k_n_b1 = 1.4 * c_k_n_b1_numerator / c_k_n_b1_denominator [cite: 90]

            ucb_values[k] = r_hat_k + c_k_n_b1 [cite: 90]

        # Select the arm with the maximum UCB value [cite: 90]
    ...
    def update_state(self, chosen_arm, cost, reward):
        ...
        """
        self.arm_pulls[chosen_arm] += 1 [cite: 85]
        self.total_costs[chosen_arm] += cost [cite: 85]
        self.total_rewards[chosen_arm] += reward [cite: 85]
